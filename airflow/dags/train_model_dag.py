...
with DAG('train-model', default_args=default_args, schedule_interval='0 0 * * *') as dag:
    partition_data = BashOperator(
        ...
        bash_command="/opt/spark/bin/spark-submit --master 'spark://master:7077' /app/partition.py"
    )
    train_model = BashOperator(
        task_id='train_model',
        bash_command="/opt/spark/bin/spark-submit --master 'spark://master:7077' /app/train.py"
    )
    test_model = BashOperator(
        task_id='test_model',
        bash_command="/opt/spark/bin/spark-submit --master 'spark://master:7077' /app/test.py"
    )
